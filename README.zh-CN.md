...
ml tool/mylocate README  # 含斜杠的关键词改为比对完整路径，用来缩小范围
ml '*.pdf'               # 通配符，整段对齐完整路径
ml "$PWD/*.pdf"          # 当前目录树下的所有 pdf
ml -w codes tool         # 所有关键词都比对完整路径
ml -b '*.pdf'            # 反过来，所有关键词都只比对文件名
ml -t d node_modules     # 只要目录（-t f 只要文件）
ml -c README             # 只打印命中数量
ml -0 README | xargs -0  # 以 NUL 分隔，文件名含换行时才安全
ml -n 0 README           # 不限条数（默认 50，-l 同义）
...
## 与 locate / plocate 的差异

旗标尽量沿用 locate 的拼法（`-b`、`-w`、`-0`、`-c`、`-i`、`-l`），通配符规则
也一致：关键词含 `*`、`?`、`[` 就整段对齐完整路径，`*` 会跨过斜杠。

但有三个默认值刻意相反，因为 mylocate 对齐的是 Everything 的手感：

| | mylocate | locate / plocate |
|---|---|---|
| 默认比对对象 | 文件名（`-w` 可改成完整路径） | 完整路径（`-b` 可改成文件名） |
| 多个关键词 | AND | OR（`-A` 才是 AND） |
| 大小写 | 一律不敏感 | 默认敏感（`-i` 才不敏感） |

`-i` 收下但不做事，只是为了让从 locate 过来的人不会被挡。索引新鲜度也不同：
locate 系列靠计划任务跑 `updatedb`，多数发行版一天一次；mylocate 靠 FSEvents 实时更新。
...
- 结果的显示顺序是目录在前、文件在后，`-n` 的配额也由目录先取。daemon 起停
  不会改变任何一条命令的输出。
- 通配符的速度取决于样式**结尾**。以字面结尾的（`*.pdf`）会拿 `.pdf` 当
  memmem 的锚点，跟普通查询一样快；以通配符结尾的（`*/keep/*`、`*.pdf*`）
  没有锚点可用，只能逐条扫描并拼出完整路径，大约慢一个数量级。
